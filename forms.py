...
class FormLogin(FlaskForm):
    usuario = StringField('Usuario',validators=[validators.required(message="Esciba el usuario"),
    validators.length(max=20)])
    password = PasswordField('Contraseña',validators=[validators.required(message="Esciba la contraseña"),
...
class FormRegistro(FlaskForm):
    usuario = StringField('Usuario',validators=[validators.required(message="Esciba el usuario"),
    validators.length(max=20)])
    password = PasswordField('Password',validators=[validators.required(message="Escriba la contraseña"),
    validators.length(max=50, min=10)])
    repassword = PasswordField("Confirmar Password", validators=[validators.required(message="Re escriba la contraseña"),
    validators.length(max=50, min=10),])
    enviar = SubmitField("Registrar")

class FormEmpleado(FlaskForm):
    tipoIdentificacion = StringField('Tipo de Identificacion',validators=[validators.required()])
#    identificacion = StringField('Número de Identificación',validators=[validators.required(mensaje="Esciba la identificación"),
#    validators.length(max=20)])
    nombre = StringField('Nombre del Empleado',validators=[validators.required(message="Esciba la identificación"),
    validators.length(max=50, min=5)])
    correo = StringField('Correo Electrónico', validators=[validators.required(message="El correo electrónico es obligatorio"),
...
#    Identificacion = PasswordField('Identificacion',validators=[validators.requerid(message="Escriba su numero de Identificacion")])
#    Area = StringField('Area',validators=[validators.requerid(message="Por favor ingrese el area, a la cual pertenece")])
